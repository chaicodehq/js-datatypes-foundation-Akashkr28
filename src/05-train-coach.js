...
export function getPassengerIndex(passengers, name) {
  ...
  return passengers.findIndex(p => typeof p.name === "string" && p.name.toLowerCase() === name.toLowerCase());
}

export function isAnyWaitlisted(passengers) {
  // Your code here
  if (!Array.isArray(passengers) || typeof passengers.length === 0) return false;
